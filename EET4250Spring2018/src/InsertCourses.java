...
public class InsertCourses {
	...
	public void inserts(Connection connection) {
		...
		String sql = "INSERT INTO Course ( " + " CourseId, InstructorId, CreditHrs, Semester, Year ) "
				+ " VALUES (?, ? ,?, ?, ?) ";
	...
	public void inserts(Connection connection, Course course) {
		...
		try {
			...
			ps.setInt(3, course.getCreditHrs());
			ps.setString(4, course.getSemester());
			ps.setInt(5, course.getYear());
			rowsAffected = +ps.executeUpdate();
		} catch (SQLException e) {
			logger.error(e.toString());
		}
